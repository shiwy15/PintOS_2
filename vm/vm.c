...
/* 주어진 가상 주소(va)에 해당하는 페이지를 보조 페이지 테이블(spt)에서 찾는 함수*/
...
{
	bool success = vm_alloc_page(VM_ANON | VM_MARKER_0, addr, true); // Create uninit page for stack; will become anon page
	if (success == true)
	{
...
/* 페이지를 할당하고 MMU(Memory Management Unit)를 설정하는 함수*/
static bool
...
/*------------------ pjt03 JK---------------------*/
/* 주어진 hash_elem의 hash값을 계산해주는 함수 */
static uint64_t hash_func(const struct hash_elem *e, void *aux)
{
...
/* 해시 테이블의 요소(page)들의 주소값을 비교하여 정렬 순서를 결정하는 데 사용되는 함수*/
static bool less_func(const struct hash_elem *a, const struct hash_elem *b, void *aux)
{
